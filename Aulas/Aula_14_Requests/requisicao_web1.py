...
#pip install requests //comando para instalação do "Requests" recomendado venv

import requests
class Conecta:
    @staticmethod
    def acessar_api(moeda_padrao, moeda_destino):
    ...
    def pegar_valores_moedas():
        ...
        for moeda in moedas:
            valor_moeda = Conecta.acessar_api(moeda_padrao, moeda)
            print(f"{moeda}: {valor_moeda}")


#forma tradicional do python para começar um sistema
if __name__ == "__main__":
    # chamada correta: método estático sem parâmetros externos
    Conecta.pegar_valores_moedas()